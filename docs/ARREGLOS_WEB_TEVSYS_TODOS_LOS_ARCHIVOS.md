...
## Complemento abr 2026 — header escáner en micros producto

- **`Header.astro`:** `showHeaderScanner` si `/`, `/features/*`, `/como-funciona`, `/precios`; clases `tevsys-header--with-scanner` + (solo home) `tevsys-header--home-scanner`; capa `.tevsys-header-scanner`; overflow del header cuando aplica.
- **`global.css`:** selectores `#odysseyNavHeader.tevsys-header--with-scanner` (base + z-index nav); refuerzo solo `.with-scanner.home-scanner` (home); bloque **Micros producto** `.with-scanner:not(.home-scanner)` con opacidades/duraciones afinadas tras prueba en terminal (segundo pase por legibilidad).
- **Docs:** `CHANGELOG-TEVSYS.md`, `CONTENIDO_WEB_TEVSYS_LANDING.md` § Header, `MOTION_HOME_TEVSYS_HANDOFF_IA.md` §1.1; repo TGP `CONTENIDO_WEB` §14.
...
| `docs/CHANGELOG-TEVSYS.md` | **Abr 2026:** entrada iteración hero/cards. **19 abr 2026:** entrada **“Web — coherencia tipográfica…”** + corrección orden Nasdaq en entrada histórica Precisión. **20 abr 2026:** patrón micros + tono comercial (§13–§16, incl. Evidencia y SML). **30 abr 2026:** *Header — escáner gris en micros producto* ampliado (rutas, CSS, dos pasadas de visibilidad). |
| `docs/CONTENIDO_WEB_TEVSYS_LANDING.md` | **Abr 2026:** hero/header/cards. **19 abr 2026:** motion escáner, DM Sans H1, stack `#000`, contacto Inter, CTAs, theme oscuro, respiro micropáginas, estado Precisión (orden demos). **20 abr 2026:** estado Evidencia (copy, CTA, meta, enlace logs); estado SML (copy §16). **30 abr 2026:** § Header — escáner en micros + opacidades/duraciones; paquete operativas 80 / precision / hyperclose. |
| `docs/CARDS-NEXT-PHASE.md` | **Abr 2026:** bloque estado + siguiente fase. **19 abr 2026:** mismo hito documentado. |
| `docs/MOTION_HOME_TEVSYS_HANDOFF_IA.md` | **19 abr 2026:** duraciones escáner header actualizadas (29,5s / 28s / 23,5s); nota “intensidad restaurada”. **30 abr 2026:** §1.1 tabla escáner en micros producto (opacidades/duraciones vs home). |
...
| `src/components/core/Header.astro` | Estilos del CTA en nav (botón blanco redondeado). **Abr 2026:** Inter, barra más baja, nav refinada. **30 abr 2026:** escáner en `/` + `/features/*` + `/como-funciona` + `/precios` (`tevsys-header--with-scanner`; solo `/` también `tevsys-header--home-scanner`); capa `.tevsys-header-scanner`; motion en `global.css`. |
...
| `docs/MOTION_HOME_TEVSYS_HANDOFF_IA.md` | **Abr 2026:** handoff IA→IA — tabla duraciones/colores/capas motion home; sincronía / asincronía. **21 abr 2026:** nota fluidez móvil (escáner + franja + rise). |
| `src/styles/global.css` | **Abr 2026:** motion home tema oscuro (KITT, burbujas, franjas, escáner header gris, `prefers-reduced-motion`, móvil). **16 abr 2026:** mid-glow móvil `*-mobile` (92s / 118s). **19 abr 2026:** stack hero **`#000`**; escáner header **intensidad restaurada** (duraciones **29,5s** base, **28s** ≥769px, **23,5s** móvil); **CTAs demo** unificados (hero, feature, precios, plan primario); **`.feature-hero`** respiro global en micropáginas. **21 abr 2026:** KITT **fall** — bienvenida **18s** + `ease-in-out` en `tevsys-home-fall-welcome-pos` (menos brusco el primer barrido vertical al cargar). **Mismo día (≤768px):** escáner header más pasos + `linear` (42s, haz más ancho); franja planes `kitt-pos-mobile` + mix 18s; rise **26s** / opacidad baja; hero mano padding+radio móvil (`HomeHeroSection.astro`). **30 abr 2026:** escáner header con **`tevsys-header--with-scanner`** (home + micros producto); home mantiene **`home-scanner`**; micros `:not(.home-scanner)` con opacidad/duración propias (segundo ajuste por visibilidad). |
