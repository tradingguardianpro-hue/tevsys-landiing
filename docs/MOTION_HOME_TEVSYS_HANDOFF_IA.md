...
| E1 | `.tevsys-home-mid-glow` (capa principal; 1 radial gris) | `tevsys-home-mid-bubbles-a` | **28s** | `ease-in-out` `infinite` (**sin** `alternate`; 0%=100%) | **Escritorio:** trayectoria orgánica (diagonal / horizontal / vertical), cuatro cards + SML/Evidencia bajo. **Móvil:** `*-a-mobile` misma filosofía en columna. |
| E2 | `.tevsys-home-mid-glow::before` (segunda burbuja / aureola) | `tevsys-home-mid-bubbles-b` | **33.7s** | `ease-in-out` `infinite` (**sin** `alternate`; `animation-delay` **-11.3s** desktop / **-15.7s** móvil) | Ruta distinta y más “nerviosa” en %; cruces con E1; duración no múltiplo de 28s. |
