...
| E1 | `.tevsys-home-mid-glow` (capa principal; 1 radial gris) | `tevsys-home-mid-bubbles-a` | **28s** | `ease-in-out` `alternate` | **Escritorio:** `@keyframes` irregulares; trayectoria tipo **perímetro** del bloque 2×2: visita **SML** (inferior izq.) y **Evidencia** (inferior dcha.) además de laterales y fila baja. **Móvil (≤768px, `prefers-reduced-motion: no-preference`):** `tevsys-home-mid-bubbles-a-mobile` — mismo periodo; recorrido centrado en **X** (~44–56%) y **Y** a lo largo de la **columna** de cards. |
| E2 | `.tevsys-home-mid-glow::before` (segunda burbuja / aureola) | `tevsys-home-mid-bubbles-b` | **33.7s** | `ease-in-out` `alternate` | **Escritorio:** ruta distinta a E1 + duración **no** múltiplo trivial de 28s → asincronía **real**. **Móvil:** `tevsys-home-mid-bubbles-b-mobile` (misma duración; fase/ruta distinta a E1m). |
...
- **Móvil / reduced motion:** reglas al final de `global.css` reducen niebla, apagan KITT rect, **escáner header activo** en home (resaltado), mid-glow con blur/opacidad propios; **`animation-name`** del mid-glow en móvil = keyframes **`*-mobile`** (columna detrás de cards). `prefers-reduced-motion: reduce` también anula **`mid-glow::before`**. Revisar `@media (prefers-reduced-motion: reduce)` y `@media (max-width: 768px)`.
