...
| E1 | `.tevsys-home-mid-glow` (capa principal; 1 radial gris) | `tevsys-home-mid-bubbles-a` | **62s** | **`linear`** `infinite` (**sin** `alternate`; 0%=100%) | Pocos pasos, recorrido moderado (sin atravesar viewport). **Móvil:** `*-a-mobile` **62s** `linear`. |
| E2 | `.tevsys-home-mid-glow::before` (segunda burbuja / aureola) | `tevsys-home-mid-bubbles-b` | **81s** | **`linear`** `infinite` (**sin** `alternate`; delay **-21.4s** desktop / **-25.1s** móvil) | Asincronía con E1; sin acelerones entre claves. |
