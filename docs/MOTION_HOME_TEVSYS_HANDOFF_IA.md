...
| E1 | `.tevsys-home-mid-glow` (capa principal; 1 radial gris) | `tevsys-home-mid-bubbles-a` | **54s** | `cubic-bezier(0.42,0,0.58,1)` `infinite` (**sin** `alternate`; 0%=100%) | Trayectoria orgánica lenta (diagonal / horizontal / vertical), cuatro cards + SML/Evidencia bajo. **Móvil:** `*-a-mobile` **54s**. |
| E2 | `.tevsys-home-mid-glow::before` (segunda burbuja / aureola) | `tevsys-home-mid-bubbles-b` | **71s** | mismo easing `infinite` (**sin** `alternate`; delay **-23.7s** desktop / **-27.4s** móvil) | Asincronía con E1 (71 no múltiplo de 54); ritmo calmado. |
...
| G | `.tevsys-home-tail-bubble-solo` | `tevsys-home-tail-bubble-solo` (**56s**); en **≥769px** `tevsys-home-tail-bubble-solo-desktop` | **56s** | `cubic-bezier(0.42,0,0.58,1)` `alternate` | Burbuja radial; zigzag suave; máscara inferior. Escritorio: misma trayectoria, opacidad/radial algo más presentes. |
