...
| E1 | `.tevsys-home-mid-glow` (capa principal; 1 radial gris) | `tevsys-home-mid-bubbles-a` | **28s** | `ease-in-out` `alternate` | **Escritorio:** perímetro 2×2 + pasos en **Y ~88–96%** para el **cuerpo gris** bajo las imágenes (SML/Evidencia y fila superior). **Móvil:** `tevsys-home-mid-bubbles-a-mobile` — columna + pasos extra en Y bajo (~78–98%). |
...
| F | `.tevsys-home-lower-strip` | `tevsys-home-lower-strip-fog` + `tevsys-home-lower-strip-kitt-pos` + mix | **18s** fog/kitt; **10.3s** mix | `ease-in-out` `alternate` (fog/kitt) | Niebla en capa base; KITT gris/ámbar en pseudo-elementos (misma idea que fall). **Móvil:** mix `tevsys-home-lower-kitt-mix-gray-mobile` / `-amber-mobile` + gradientes algo más intensos (misma gama). |
