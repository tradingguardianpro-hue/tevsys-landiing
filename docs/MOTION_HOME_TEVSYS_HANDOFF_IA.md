...
| E | `.tevsys-home-mid-glow` (doble radial = “dos burbujas”) | `tevsys-home-mid-bubbles` | **22s** | `ease-in-out` `alternate` | `background-position` **y** `background-size` **y** `opacity` en keyframes → trayectoria ancha tipo zigzag/diagonal. Sin máscara central (revertido: prioridad ver movimiento detrás de cards). |
