...
- **21 abr 2026 (móvil, fluidez):** escáner `tevsys-header-scanner-sweep-mobile` con **más keyframes** + **56s** `linear`; franja inferior con **`tevsys-home-lower-strip-kitt-pos-mobile`** (barrido vertical en pasos) + mezclas **18s** `linear` y opacidades más graduales; hero **rise** a **26s** en niebla/KITT. Objetivo: menos sensación de “trozos” en compositor móvil.
