...
- **Commit:** `web(tevsys): align alcanzado huecos lexicon on vigilancia-vps`
...
- **Commit:** `web(tevsys): clarify MAM/PAMM as multi-account manager on para-quien`
...
- **Commit captura:** `web(tevsys): hang Tu ID TEVsys modal capture on como-funciona`
...
- **Qué:** bajo **Elige tu cuenta** → *No vendemos señales. Vendemos reglas que se ejecutan y se pueden contrastar en MT5.*
- **Antes:** lista larga (ejecución disciplinada / precisión / evidencia / presión).
- **Commit:** `web(tevsys): tighten plans subtitle to rules you can verify`
...
- **Por qué:** hub Market EN (775054) enlazó “evidence”; canónica web = `evidencia`.
- **Commit:** `web(tevsys): redirect features/evidence to evidencia`
...
**Commit:** `web(tevsys): precision carril A poda copy logs recaptura note`
...
**Ajuste aire (15 ago):** más hueco entre tokens y marco (shell más estrecho / stage más ancho); **sin** bajar opacidad.

**Commit:** `web(tevsys): home audit rain fuera del marco`
**Commit aire:** `web(tevsys): home audit rain mas aire flancos`
...
## Hero — chevron a ~0,5 s (26 jul 2026) — **paso 4b**

**Qué:** ~0,5 s de aire al cargar (no al instante) → luego mismo blur del chevron. Frases siguen retrasadas para juzgar la marca.

**Commit:** `web(tevsys): hero chevron arranca a medio segundo`
...
**Commit:** `web(tevsys): sync contador 103 operativas documentadas`
...
**Commit:** `web(tevsys): hero disponible MT5 sin mas plataformas`
...
**Commit:** `web(tevsys): hero marca smooth y fondo negro puro`
