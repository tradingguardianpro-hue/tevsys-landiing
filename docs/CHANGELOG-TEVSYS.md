...
**Resumen numérico (dark, `prefers-reduced-motion: no-preference`):** niebla home `body::before` **23.3s**; rise **18.7s**; fall **311s** (`--tevsys-home-fall-cycle`); escáner header gris **33.8s** (bucle irregular, `ease-in-out` por tramo); **mid-glow** burbuja A **28s** + burbuja B (`::before`) **33.7s** (asincronía real; ver iteración 15–16 abr abajo); lower strip **18s**; tail bubble **38s**; founder strip **16s**. Variables `--tevsys-home-intro-cycle` 300s vs fall 311s para no ir a la par.
