...
| **Cards home** | Nuevos PNG en `public/.../card-*.png`; **cache-bust `?v=7`** en `index.astro`. Cintas retocadas en origen (tonos ámbar/gris/negro, sin arcoíris). **Lectura:** `feature-card__value-accent` en cuerpo — **71 operativas**, **no poder seguir**, **mín. 75 % protegido** (misma línea que evidencia `0,0072 %`). | `index.astro`, assets |
