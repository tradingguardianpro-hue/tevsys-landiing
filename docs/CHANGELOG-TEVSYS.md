...
**Resumen numérico (dark, `prefers-reduced-motion: no-preference`):** niebla home `body::before` **23.3s**; rise **18.7s**; fall **311s** (`--tevsys-home-fall-cycle`); escáner header gris **33.8s** (bucle irregular, `ease-in-out` por tramo); **mid-glow** burbuja A **62s** + burbuja B (`::before`) **81s** (`linear`, pocos hitos; ver iteración 15–16 abr abajo); lower strip **18s**; tail bubble **64s** `linear`; founder strip **16s**. Variables `--tevsys-home-intro-cycle` 300s vs fall 311s para no ir a la par.
...
| **Calma / premium** | Mid-glow **62s / 81s** + **`linear`** y trayectorias **reducidas** (el ease-in-out entre claves lejanas producía “sprints”). Tail **64s** `linear`. `prefers-reduced-motion: reduce` apaga también animación del `::before`. |
