...
- **MC-CF-025 (18 ago):** `#videoclip-limite-hyperclose` · rama 2 · ~34,5 s · `tevsys-mc-limite-hyperclose-30s.mp4`. Overlays: `¿Qué pasa si insistes después de alcanzar tu límite?` · `➜ Límite de ganancia alcanzado` · `🡰 STOP` · `Intentas abrir otra vez` · `Los siguientes intentos quedan trazados.` Commit: `web(tevsys): publish limite-hyperclose microclip on como-funciona`.
