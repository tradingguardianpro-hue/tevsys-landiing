...
- **CTA secundario + lead (22 ago):** botón **`Nasdaq 100 — sesión real (~3:35)`** → `/como-funciona?play=1#evidencia-nasdaq-100` (sin párrafo extra encima del botón). MP4 `?v=20260822` · sesión **6263709** · Transparencia + **Atribución** (sin nombre de bróker).
- **Presentación `/como-funciona#evidencia-nasdaq-100` (22 ago):** H2 *Cuatro minutos de sesión real en MT5* + sub original (precisión y evidencia · *Contrasta tú mismo*) · blurb corto bajo título del vídeo · Transparencia + **Atribución** (sin nombre de bróker en copy público). Overlays: `AGENDA_GABI` § *Overlays definitivos 21 ago*.
