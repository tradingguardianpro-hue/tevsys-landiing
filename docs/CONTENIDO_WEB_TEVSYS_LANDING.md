...
- **Mid-home microclips (23 ago 2026 · chip 29 ago · MP4 29 ago):** tras las 4 cards — *¿Quieres ver cómo reacciona…?* · lead: Nasdaq (~3:35) · clips ~30 s · guías · **chip** *Operación abierta en fin de semana — sigues protegido* → `/go/como-funciona-operacion` → `#rama-operacion` (`#videoclip-finde-operacion` = **MC-CF-022** MP4 ~33 s) · CTA `Ver cómo funciona →` → `/go/como-funciona-home`.
