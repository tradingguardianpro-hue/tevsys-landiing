...
- **Tu ID tevsys (9 sep 2026):** `/como-funciona#tu-id-tevsys` ES|EN — lead del acto Auditoría (tras corte *fase de auditoría*). Identidad del caso / código `TVS-…`. Captura `tevsys-modal-tu-id-tvs.png` (1024×540; ideal re-captura ~1900px). Handoff: `HANDOFF_SESION_9SEP2026_COPY_HOME_CF_ID_OUTREACH.md`.
- **Home definición (9 sep):** 3.º beat *acta desde la instalación* · línea *Atribución clara…* · subrayado *lo demuestra, operativa a operativa*. Mismo handoff.
- **Actos post-Capital (30 ago · update 11 sep):** … → auditoría → ~~*De cero a protegido* placeholder~~ **retirado de la web** (concepto en `zeroToProtected` para guión futuro) → Vigilancia / FAQ… Guía **Desde cero** sigue. V12 **(cfZeroProtectedOff)**.
...
- **P13 + pack espejo + GEO AI (8–9 sep):** `#rama-operacion` · **`#overnight-laborable`** · `#overnight-faq` · `#tu-id-tevsys` · `llms.txt` reforzado (V12 **geoEspejoAI**). Pendiente caso ★ · re-auditoría buscador 2–3 sem tras push.
...
**P0 siguiente (acordado 11 ago 2026):** **regrabar pieza Nasdaq** — **✅ web + YT 22 ago 2026** (`6263709`, ~3:35) · **https://youtu.be/sAttywGy0nQ** *(hist. `2f1rRmqkE1k` may)*.
...
- Cita (12 ago 2026): `"Nadie me enseñó a gestionar el riesgo ni la disciplina. Tuve que inventar algo que me obligara a cumplir mis límites — con fiabilidad, con precisión, y sabiendo qué pasaba en mi cuenta. tevsys es eso: lo que me faltaba entonces, ahora para todos."`
...
- **Sección "Auditoría":** **101** operativas documentadas (alineado a hook Precisión / resumen estadístico), incluyendo caso extremo 100 lotes (operativa 4.56). Hook "MetaTrader documenta. tevsys ejecuta."
